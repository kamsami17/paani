<?php
  ...
  echo '<input type="hidden" value="'.($page+1).'" id="CURR_PAGE_clas" />'; 
	...
	$query  = 'SELECT * FROM classe WHERE ecole='.$_SESSION['user']->ecole.(isset($_POST['search'])? ' AND '.htmlspecialchars($_POST['option']).' LIKE \''.htmlspecialchars($_POST['search']).'%\'' : '');
...
<div class="data_area_clas col s12">
	<div class="col s12">
		<div class="col m6 l6"><font class="flow-text">Gestion des classes</font></div>
	...
	<table class="col s12 striped">
	  ...
	  <tbody>
		<?php
		  ...
		  $entity->psize($query, 'CLASSE_SIZE');
		?>
	  </tbody>
	</table>
	<div class="CLASSE_FOOT col s12"></div>
</div>
...
<div class="col s12 add_area_clas" style="display: none;">
	<div class="col s12">
	  ...
	  <div class="col l10 m8 s6"><font class="flow-text">Ajouter/Modifier classe</font></div>
    </div>
...
<style> .label_detail { font-weight: bold; } </style>
    ...
    <script type="text/javascript">
		function init_clas(pg){
          pg = pg || 0;
          $('.LOADER').show();
          $.post('../php/modules/classe/classe_util.php',
            {page: pg},
            function(data){
              $('.LOADER').hide();
              $('.load_area').html(data);
      ...
      $(document).ready(function(){
		...
		$('.CLASSE_FOOT').pagination({
			items: parseInt($('#CLASSE_SIZE').val()),
          ...
          //$('.CLASSE_FOOT').pagination('selectPage', parseInt($('#CURR_PAGE').val()));
        ...
        $('.bt_delete_clas').click(function(){
          swal({
            ...
            if(del){
              $('.LOADER').show();
              $.post(
                '../php/modules/classe/classe_data.php',
		...
		//   );
			$('.LOADER').show();
			$.post(
				'../php/modules/classe/open_classe.php',
				{ id: $(this).attr('data') },
				function(data){
					$('.LOADER').hide();
					$('.load_area').html(data);
        ...
        $('#formAdd_clas').submit(function(e){
          ...
          $.ajax({
              url : '../php/modules/classe/classe_data.php',
              data : fd,
              type : 'POST',
              processData: false,
              contentType: false,
